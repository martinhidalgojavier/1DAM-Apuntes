...
- **`border` (en desuso)**:  
  En versiones antiguas de HTML se utilizaba el atributo `border` para mostrar el borde de la tabla, por ejemplo:

```html
  <table border="1"> <!-- Obsoleto en HTML5 -->
```

Aunque aún funciona, este atributo está **en desuso (deprecated)** y los editores modernos lo marcan en rojo.
En HTML5 se recomienda definir los bordes usando **CSS**.
...
Esta forma es válida en HTML5 y produce el mismo resultado visual, además de permitir más control sobre el grosor, color y estilo del borde. Se compone de tres partes:

1px → grosor del borde (1 píxel).

solid → tipo de línea (continua; también puede ser dashed, dotted, etc.).

black → color del borde (negro).
...
```html
<table style="border: 1px solid black;" cellpadding="5" cellspacing="2" width="50%">
  <tr>
    <th>Producto</th>
    <th>Precio</th>
  </tr>
